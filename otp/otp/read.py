...
class SRAM16x16:
    ...
    def __init__(self, dir_fVec, fMap):
        ...
        self.dictVec = {}
        self.dictVec_df = {}
        for filename in os.listdir(dir_fVec):# 列出当前路径,返回字符串在列表里
            f = os.path.join(dir_fVec, filename) # 组成路径
            if os.path.isfile(f):#判断路径有效性
                if self.all0 in f:
    ...
    def commonSetting(self, sess : nidcpower.Session):
        ...
        df = self.dictVec_df[self.chkbrd1]
        pinMap = self.pinMap
    ...
    def populate(self, sess : nidcpower.Session):
        # df = self.df
        pinMap = self.pinMap
        for sequenceName in [self.chkbrd1]:
            ...
            for iCyc in range(numCycle):
                for iPhase in range(1):
                    ...
                    for sigName in df.columns:
                        ...
                        chn.aperture_time = aperture_time
                        chn.current_limit_range = 200e-6
                        chn.current_limit       = 200e-6
    ...
    def testSRAM(self, session: nidcpower.Session, x, y, foutDIR):
        for sequenceName in [self.chkbrd1]:
            ...
            with contextlib.ExitStack() as ctxt:
                ...
                with open(f'{foutDIR}OTPBB__{x}_{y}_{sequenceName}_{csv_name}.csv', 'w', newline='') as fout:
